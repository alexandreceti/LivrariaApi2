...
var LivrosModel = require('./livros.model')
// const User = require('../models/users')
const api = {}

api.list = async (request, h, error) => {
  /** Lista de albums do usuarios no sistema.
  * @param {String} query pesquisa
  * @param {Number} page Sobre nome do usuario
  * @param {Number} pagesize quantidade por pagina
  * @param {String} order Senha do usuario
  */
...
api.create = async (request, h, error) => {
  ...
  try {
    LivrosModel.lista.push(new_livro)
    return new_livro
  } catch (err) {
    console.log(err)
...
api.show = async (request, h, error) => {
  ...
  console.log(isbn)
  try {
    let busca = LivrosModel.lista.filter((item) => item.isbn === isbn)
    console.log(busca)
    return busca[0]
  } catch (err) {
    console.log(err)
