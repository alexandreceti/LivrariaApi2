...
var pedidosModel = require('./pedidos.model')
...
api.create = async (request, h, error) => {
  ...
  try {
    pedidosModel.lista.push(new_livro)
    return new_livro
  } catch (err) {
    console.log(err)
...
api.show = async (request, h, error) => {
  ...
  console.log(isbn)
  try {
    let busca = pedidosModel.lista.filter((item) => item.isbn === isbn)
    console.log(busca)
    return busca[0]
  } catch (err) {
    console.log(err)
