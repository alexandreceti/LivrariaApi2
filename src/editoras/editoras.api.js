...
var editorasModel = require('./editoras.model')
...
api.create = async (request, h, error) => {
  ...
  try {
    editorasModel.lista.push(new_livro)
    return new_livro
  } catch (err) {
    console.log(err)
...
api.show = async (request, h, error) => {
  ...
  console.log(isbn)
  try {
    let busca = editorasModel.lista.filter((item) => item.isbn === isbn)
    console.log(busca)
    return busca[0]
  } catch (err) {
    console.log(err)
